    ...
    <KonvaGroup key={key} x={0} y={0}>
      {pixels.toPlacements().map((pixel, index) => (
        <Rect
          ...
          width={scale}
          height={scale}
          fill={`rgba(${pixel.pixel.r}, ${pixel.pixel.g}, ${pixel.pixel.b}, ${(pixel.pixel.a ?? 255) / 255})`}
        />
      ))}
    </KonvaGroup>
  );
  ...
  return (
    <>
      {Array.from({ length: width }, (_, x) => (
        <Rect key={x} x={x * scale} y={0} width={1} height={height * scale} fill="rgba(0, 0, 0, 0.1)" />
      ))}
      {Array.from({ length: height }, (_, y) => (
        <Rect key={y} x={0} y={y * scale} width={width * scale} height={1} fill="rgba(0, 0, 0, 0.1)" />
      ))}
    </>
  );
}

function KonvaRenderer({ canvasWidth, canvasHeight, scale = 1, scene, addGrid = true }: KonvaRendererProps): ReactNode {
  return (
    <Stage width={canvasWidth * scale} height={canvasHeight * scale}>
      <Layer>
        {/* Fill */}
        <Rect x={0} y={0} width={canvasWidth * scale} height={canvasHeight * scale} fill="white" />
        {/* Grid */}
